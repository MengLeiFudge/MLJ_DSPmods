...
资源复制协议沿用资源本身的阶段分类。不可自动采集的木材和植物燃料归电磁阶段；可燃冰允许进入能量阶段；必须依赖星际航行的其他珍奇矿物不得提前。转化协议按全部主产物和副产物中的最高主线矩阵阶段归类，避免低阶输入提前开放高阶输出。
...
`MineralCopyRecipe.Create` 明确排除临界光子和反物质。资源塔基础配方只生成输入资源的同种复制，不附送会跨越原版生产边界的稀有副产物。
...
| 资源塔 | `MineralCopy` | 同种原矿复制 |
| 转化塔 | `Conversion` | 工业链或用途链转换 |
...
- 不让 `MineralCopy` 创建临界光子、反物质或输入资源之外的关键稀有产物。
