...
当前主面板按玩家可见功能分为 6 个业务子域。根层只放面板壳、路由、契约与共享状态；业务目录里默认 **一个页面一个 `.cs` 文件**。核心变化：**双主面板架构**、**抽取成长页重构**，且符文页已移除。
...
7. 页面过大时，优先把可复用的规则、计算、格式化、数据模型下沉到对应 `Logic/*` 功能域或 `UI/Components`，不要用 partial 文件掩盖页面承担过多职责。
