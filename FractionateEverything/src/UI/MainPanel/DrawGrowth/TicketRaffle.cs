...
public static class TicketRaffle {
    ...
    public static void AddTranslations() {
        Register("主抽取", "Main Retrieval", "主检索");
        Register("主抽取开线方向", "Main Retrieval - Route", "主检索路线方向");
        Register("主抽取原胚方向", "Main Retrieval - Proto", "主检索原胚方向");
        Register("成长说明", "Growth Info");
        Register("聚焦说明", "Focus Info");
        Register("常规模式", "Normal Mode");
        Register("速通模式", "Speedrun Mode");

        Register("主抽取开线偏好", "Main Retrieval - Route Preference", "主检索：路线偏好");
        Register("主抽取原胚偏好", "Main Retrieval - Proto Preference", "主检索：原胚偏好");
        ...
        Register("主抽取开线偏好说明",
            "Main retrieval is currently steering toward route units such as resource groups, conversion chains, and rectification families. Focus raises matching directions.",
            "主检索当前偏向资源组、转化链、精馏家族等路线检索单位。当前聚焦会提高对应方向的命中权重。");
        Register("主抽取原胚偏好说明",
            "Main retrieval is currently steering toward tower protos. Focus raises matching tower embryos.",
            "主检索当前偏向各类塔型原胚。当前聚焦会提高对应塔种原胚的出现权重。");
        Register("成长规划说明",
            "Growth is deterministic. Use calibration progress, fragments, and matrix essences on the Growth page.",
            "成长规划为非随机成长入口。请使用校准进度、残片与矩阵精华进行补差和定向成长。");
        Register("流派聚焦说明",
            "Focus is not a standalone retrieval domain. Select a focus on the Focus page to bias Main Retrieval and Growth Planning.",
            "流派聚焦不是独立检索域。请前往聚焦页选择方向，以偏置主检索和成长规划的奖励分布。");
        Register("速通开线方向说明",
            "Speedrun mode only. Each retrieval directly yields a current-stage route target, greatly accelerating route unlock speed.",
            "仅速通模式使用。每次检索都直接给当前阶段的路线目标，显著提升路线成型速度。");
        Register("速通原胚方向说明",
            "Speedrun mode only. Proto growth is more aggressive and tower protos converge faster.",
            "仅速通模式使用。原胚成长节奏更激进，塔型原胚成型更快。");
        Register("速通成长规划说明",
        ...
        Register("保底进度", "Stabilization");
        Register("当前池积分", "Calibration Progress", "校准进度");
        Register("成长积分", "Calibration Progress", "校准进度");
        Register("抽1次", "Retrieve x1", "检索1次");
        Register("抽10次", "Retrieve x10", "检索10次");
        Register("抽取偏好", "Retrieval Preference", "检索偏好");
        ...
        Register("抽取单位回响", "Retrieval Unit Resonance", "检索单位回响");
        ...
        Register("保底命中", "Stabilized Hit", "锚定命中");
        ...
        Register("聚焦描述-复制扩张", "Bias Mineral Replication retrieval units and Mineral Replication Tower protos.",
            "提高矿物复制检索单位与矿物复制塔原胚的出现权重。");
        ...
        Register("聚焦描述-原胚循环", "Bias tower protos and deterministic proto catch-up.", "提高塔型原胚与原胚补差收益，强化原胚循环。");
        Register("聚焦-工艺优化", "Process Optimization");
        Register("聚焦描述-工艺优化", "Bias current-stage retrieval units and Conversion Tower protos.", "提高当前阶段检索单位与转化塔原胚权重。");
    ...
    private static string GetFocusEffectSummary() {
        ...
        return GachaManager.CurrentFocus switch {
            GachaFocusType.Balanced => "不额外偏置主检索，成长规划保持原价。".WithColor(White),
            GachaFocusType.MineralExpansion => $"主检索路线偏向矿物复制检索单位；成长规划命中方向条目按 {discountPercent:0}% 成本结算。".WithColor(Green),
            GachaFocusType.ConversionLeap => $"主检索路线偏向转化链检索单位；成长规划命中方向条目按 {discountPercent:0}% 成本结算。".WithColor(Green),
            GachaFocusType.LogisticsInteraction => $"主检索路线偏向物流链检索单位，原胚方向偏向交互塔原胚；成长规划命中方向条目按 {discountPercent:0}% 成本结算。"
                .WithColor(Green),
            GachaFocusType.EmbryoCycle =>
                $"主检索偏向未解锁检索单位与塔型原胚；成长规划命中方向条目按 {discountPercent:0}% 成本并额外 +1。".WithColor(Green),
            GachaFocusType.ProcessOptimization => $"主检索路线偏向当前阶段检索单位，原胚方向偏向转化塔；成长规划命中方向条目按 {discountPercent:0}% 成本结算。"
                .WithColor(Green),
            GachaFocusType.RectificationEconomy => $"主检索路线偏向精馏家族，原胚方向偏向精馏塔；成长规划命中方向条目按 {discountPercent:0}% 成本结算。"
                .WithColor(Green),
            _ => string.Empty,
        };
    }
