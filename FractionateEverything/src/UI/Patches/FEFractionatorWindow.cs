...
public static class FEFractionatorWindow {
    ...
    // ===== 窗口实例 =====
    private static UIFractionatorWindow vanillaWindow;  // 原版窗口引用
    ...
    private static readonly ProductSlot[] vanillaMainSlots = new ProductSlot[MaxMainSlots];
    private static readonly ProductSlot[] vanillaSideSlots = new ProductSlot[MaxSideSlots];
    private static Text vanillaMainSectionLabel;
    private static Text vanillaAppendSectionLabel;
    private static Text vanillaFluidSectionLabel;
    private static Text vanillaFluidRightText;
    ...
    public static void CreateModWindowInstance(UIFractionatorWindow __instance) {
        ...
        // 为原版窗口也创建自定义元素（但初始隐藏）
        CreateVanillaCustomElements(__instance);
    }
    ...
    /// <summary>
    /// 为原版窗口创建自定义元素（用于可能的模组建筑切换场景）
    /// </summary>
    private static void CreateVanillaCustomElements(UIFractionatorWindow window) {
        Text refLabel = window.inputTitleText ?? window.titleText;
        
        vanillaMainSectionLabel = CreateLabel(window, refLabel, "主产物".Translate());
        vanillaAppendSectionLabel = CreateLabel(window, refLabel, "副产物".Translate());
        vanillaFluidSectionLabel = CreateLabel(window, refLabel, "流体输出".Translate());
        
        if (window.oriProductProbText != null) {
            GameObject fluidRightGo = Object.Instantiate(window.oriProductProbText.gameObject, window.transform);
            fluidRightGo.name = "vanilla-fluid-right-info";
            vanillaFluidRightText = fluidRightGo.GetComponent<Text>();
            vanillaFluidRightText.alignment = TextAnchor.UpperLeft;
            vanillaFluidRightText.horizontalOverflow = HorizontalWrapMode.Overflow;
            vanillaFluidRightText.verticalOverflow = VerticalWrapMode.Overflow;
            vanillaFluidRightText.supportRichText = true;
            vanillaFluidRightText.gameObject.SetActive(false);
        }
        
        for (int i = 0; i < MaxMainSlots; i++) {
            vanillaMainSlots[i] = CreateSlot(window);
        }
        for (int i = 0; i < MaxSideSlots; i++) {
            vanillaSideSlots[i] = CreateSlot(window);
        }
    }
    ...
    public static bool OnWindowClose(UIFractionatorWindow __instance) {
        ...
        // 原版窗口关闭时，隐藏自定义元素
        HideVanillaCustomElements();
        return true;
    }
    
    private static void HideVanillaCustomElements() {
        vanillaMainSectionLabel?.gameObject.SetActive(false);
        vanillaAppendSectionLabel?.gameObject.SetActive(false);
        vanillaFluidSectionLabel?.gameObject.SetActive(false);
        vanillaFluidRightText?.gameObject.SetActive(false);
        
        foreach (var slot in vanillaMainSlots) {
            slot.go.SetActive(false);
        }
        foreach (var slot in vanillaSideSlots) {
            slot.go.SetActive(false);
        }
    }
    ...
    public static void OnWindowUpdate(UIFractionatorWindow __instance) {
        ...
        if (isFirstUpdateUI) {
            ...
            if (vanillaWindow != null) {
                vanillaWindow.titleText.horizontalOverflow = HorizontalWrapMode.Overflow;
                vanillaWindow.productProbText.horizontalOverflow = HorizontalWrapMode.Overflow;
                vanillaWindow.oriProductProbText.horizontalOverflow = HorizontalWrapMode.Overflow;
                vanillaWindow.productProbText.verticalOverflow = VerticalWrapMode.Overflow;
                vanillaWindow.oriProductProbText.verticalOverflow = VerticalWrapMode.Overflow;
                productProbTextBaseY = vanillaWindow.productProbText.transform.localPosition.y;
                oriProductProbTextBaseY = vanillaWindow.oriProductProbText.transform.localPosition.y;
                vanillaWindow.productProbText.supportRichText = true;
                vanillaWindow.oriProductProbText.supportRichText = true;
            }
        }
        
        // 模组窗口的更新
        if (__instance == modWindow && modWindow.active) {
            UpdateModWindowContent(__instance);
        }
        // 原版窗口的更新（如果是模组建筑在原版窗口中显示）
        else if (__instance == vanillaWindow && vanillaWindow.active) {
            UpdateVanillaWindowContent(__instance);
        }
    }
    
    private static void UpdateModWindowContent(UIFractionatorWindow window) {
        if (window.fractionatorId == 0 || window.factory == null) return;
        
        FractionatorComponent fractionator = window.factorySystem.fractionatorPool[window.fractionatorId];
        if (fractionator.id != window.fractionatorId || fractionator.fluidId == 0) return;
        ...
        window.titleText.text = level > 0 ? $"{building.name} +{level}" : building.name;
        
        // 更新状态文本
        UpdateStateText(window, fractionator, building, buildingID);
        
        // 更新速率
        UpdateSpeedText(window, fractionator);
        ...
        UpdateUIElements(window, fractionator, building, recipe, 
            recipeSuccessRatio, mainOutputBonus, destroyRatio,
            mainSlots, sideSlots, mainSectionLabel, appendSectionLabel, 
            fluidSectionLabel, fluidRightText);
    }
    
    private static void UpdateVanillaWindowContent(UIFractionatorWindow window) {
        if (window.fractionatorId == 0 || window.factory == null) return;
        ...
        if (buildingID < IFE交互塔 || buildingID > IFE回收塔) {
            window.productProbText.transform.localPosition = new(0, productProbTextBaseY, 0);
            window.oriProductProbText.transform.localPosition = new(0, oriProductProbTextBaseY, 0);
            HideVanillaCustomElements();
            return;
        }
        ...
        UpdateUIElements(window, fractionator, building, recipe,
            recipeSuccessRatio, mainOutputBonus, destroyRatio,
            vanillaMainSlots, vanillaSideSlots, vanillaMainSectionLabel, 
            vanillaAppendSectionLabel, vanillaFluidSectionLabel, vanillaFluidRightText);
    }
    ...
    private static void UpdateUIElements(UIFractionatorWindow window,
        ...
        ProductSlot[] mainSlotArray, ProductSlot[] sideSlotArray,
        Text mainLabel, Text appendLabel, Text fluidLabel, Text fluidInfo) {
        
        List<ProductOutputInfo> products = fractionator.products(window.factory);
        bool sandboxMode = GameMain.sandboxToolsEnabled;
        
        // 隐藏所有槽位
        foreach (var slot in mainSlotArray) slot.go.SetActive(false);
        foreach (var slot in sideSlotArray) slot.go.SetActive(false);
        ...
        if (recipe != null && !recipe.Locked) {
            ...
            foreach (var output in recipe.OutputMain) {
                ...
                ProductSlot slot = mainSlotArray[mainCount];
            ...
            foreach (var output in recipe.OutputAppend) {
                ...
                var pInfo = products.Find(p => p.itemId == output.OutputID && !p.isMainOutput);
                int count = pInfo?.count ?? 0;
                ProductSlot slot = sideSlotArray[sideCount];
                slot.go.SetActive(true);
                if (slot.button != null) slot.button.data = output.OutputID;
                ItemProto itemProto = LDB.items.Select(output.OutputID);
                if (itemProto != null && slot.icon != null) slot.icon.sprite = itemProto.iconSprite;
        ...
        SetLabelPos(fluidLabel, fluidLabelX, fluidLabelY);
        fluidLabel.gameObject.SetActive(true);
        
        // 流体输出右侧信息
        if (fluidInfo != null) {
            RectTransform frRT = fluidInfo.GetComponent<RectTransform>();
            if (frRT != null) {
                float infoX = fluidRowPos.x + 60f;
                frRT.anchoredPosition = new Vector2(infoX, fluidRowPos.y + 8f);
            }
            fluidInfo.gameObject.SetActive(true);
        ...
        if (sideCount > 0) {
            ...
            for (int i = 0; i < sideCount; i++) {
                SetSlotPos(sideSlotArray[i], startX + i * SlotSpacing, sideRowY);
            }
            
            float sideLabelX = startX - SlotSpacing * 0.5f - 5f;
            SetLabelPos(appendLabel, sideLabelX, sideRowY + labelOffsetY);
            appendLabel.gameObject.SetActive(true);
        } else {
            appendLabel.gameObject.SetActive(false);
        }
        ...
        if (mainCount > 0) {
            ...
            for (int i = 0; i < mainCount; i++) {
                SetSlotPos(mainSlotArray[i], startX + i * SlotSpacing, mainRowY);
            }
            
            float mainLabelX = startX - SlotSpacing * 0.5f - 5f;
            SetLabelPos(mainLabel, mainLabelX, mainRowY + labelOffsetY);
