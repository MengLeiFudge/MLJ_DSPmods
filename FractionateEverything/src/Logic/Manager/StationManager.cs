...
public static class StationManager {
    ...
    public static void UIControlPanelStationStorage_RefreshValues_Postfix(UIControlPanelStationStorage __instance) {
        ...
        if (!TryPrepareControlPanelStorageRefresh(storage, out RectTransform localRect, out float topY,
                out float bottomY)) {
            return;
        }

        EnsureControlPanelModeButtonsCached(storage);
        ...
        RefreshControlPanelTransferButton(storage, localRect, topY, transferMode);
        RefreshControlPanelCapacityButton(storage, localRect, bottomY, transferMode);
    }

    private static bool TryPrepareControlPanelStorageRefresh(UIControlPanelStationStorage storage,
        out RectTransform localRect, out float topY, out float bottomY) {
        localRect = null;
        topY = BtnYOffset;
        bottomY = -BtnYOffset;
        ...
        ShiftControlPanelBaseButtonsLeft(storage);

        localRect = storage.localSdButton?.GetComponent<RectTransform>();
        if (localRect == null) {
            return false;
        }

        CalculateControlPanelButtonVerticalPositions(storage, out topY, out bottomY);
        return true;
    }

    private static void ShiftControlPanelBaseButtonsLeft(UIControlPanelStationStorage storage) {
        ShiftControlPanelButtonLeft(storage.localSdButton?.GetComponent<RectTransform>());
        ShiftControlPanelButtonLeft(storage.remoteSdButton?.GetComponent<RectTransform>());
        ShiftControlPanelButtonLeft(storage.keepModeButton?.GetComponent<RectTransform>());
    }

    private static void ShiftControlPanelButtonLeft(RectTransform rect) {
        if (rect == null) {
            return;
        }

        float originalX = GetOrCacheOriginal(controlPanelSdButtonOriginalPosition, rect, x => x.anchoredPosition.x);
        rect.anchoredPosition = new Vector2(originalX - spacingX, rect.anchoredPosition.y);
    }

    private static void CalculateControlPanelButtonVerticalPositions(UIControlPanelStationStorage storage,
        out float topY, out float bottomY) {
        RectTransform localImgRT = storage.localSdImage?.rectTransform;
        RectTransform remoteImgRT = storage.remoteSdImage?.rectTransform;
    ...
    private static void EnsureControlPanelModeButtonsCached(UIControlPanelStationStorage storage) {
    ...
    private static void RefreshControlPanelTransferButton(UIControlPanelStationStorage storage, RectTransform localRect,
        float topY, ETransferMode transferMode) {
        ...
        RectTransform transferRect = transferGO.GetComponent<RectTransform>();
        transferRect.anchoredPosition = new Vector2(localRect.anchoredPosition.x + spacingX, topY);
        transferRect.sizeDelta = new Vector2(transferRect.sizeDelta.x, BtnHeight);
    }

    private static void RefreshControlPanelCapacityButton(UIControlPanelStationStorage storage, RectTransform localRect,
        float bottomY, ETransferMode transferMode) {
        ...
        RectTransform capacityRect = capacityGO.GetComponent<RectTransform>();
        capacityRect.anchoredPosition = new Vector2(localRect.anchoredPosition.x + spacingX, bottomY);
        capacityRect.sizeDelta = new Vector2(capacityRect.sizeDelta.x, BtnHeight);
    }
