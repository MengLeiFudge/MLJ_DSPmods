    ...
    public static void InternalUpdate(ref FractionatorComponent __instance,
        ...
        if (__instance.fluidInputCount > 0
            ...
            for (; __instance.progress >= 10000; __instance.progress -= 10000) {
                ...
                // 量子复制塔有很高的成功率
                double baseProb = __instance.produceProb * 3.0;// 基础概率提高3倍
                double incFactor = 1.0 + 2.0 * Cargo.accTableMilli[itemIncAvg < 10 ? itemIncAvg : 10];// 增产剂效果翻倍
                ...
                if (__instance.fractionSuccess) {
                    ...
                    // 失败时的处理，量子复制塔仍有90%回收率
                    if (__instance.seed % 10 != 0) {
