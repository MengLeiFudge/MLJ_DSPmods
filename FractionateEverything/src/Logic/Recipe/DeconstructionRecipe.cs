...
public class DeconstructionRecipe : BaseRecipe {
    ...
    /// 存储所有分解配方的静态列表
    /// </summary>
    private static readonly List<DeconstructionRecipe> deconstructionRecipeList = [];

    /// <summary>
    /// 根据输入物品ID获取分解配方
    /// </summary>
    /// <param name="inputID">输入物品ID</param>
    /// <returns>对应的分解配方，如果未找到则返回null</returns>
    public static DeconstructionRecipe GetRecipe(int inputID) {
        foreach (DeconstructionRecipe r in deconstructionRecipeList) {
    ...
    public static void AddRecipe(DeconstructionRecipe recipe) {
        deconstructionRecipeList.Add(recipe);
    }

    /// <summary>
    /// 获取所有分解配方列表
    /// </summary>
    /// <returns>分解配方列表</returns>
    public static List<DeconstructionRecipe> GetAllRecipes() {
        return deconstructionRecipeList;
    }
    ...
    public static void ClearRecipes() {
        deconstructionRecipeList.Clear();
    }
    ...
    public DeconstructionRecipe(int inputItemId, float baseSuccessRate = 0.05f, int level = 1, int star = 0) {
        ...
        // 初始化原材料和精华输出
        MaterialOutputs = new Dictionary<int, float>();
        EssenceOutputs = new Dictionary<int, float>();
    }
    ...
    /// <summary>
    /// 原材料输出
    /// 结构: Dictionary<物品ID, 数量>
    /// </summary>
    public Dictionary<int, float> MaterialOutputs { get; set; } = new Dictionary<int, float>();

    /// <summary>
    /// 精华输出
    /// 结构: Dictionary<精华ID, 概率>
    /// </summary>
    public Dictionary<int, float> EssenceOutputs { get; set; } = new Dictionary<int, float>();
    ...
    /// 添加原材料输出
    /// </summary>
    /// <param name="materialId">原材料ID</param>
    /// <param name="count">数量</param>
    public void AddMaterialOutput(int materialId, float count) {
    ...
    /// 添加精华输出
    /// </summary>
    /// <param name="essenceId">精华ID</param>
    /// <param name="probability">概率</param>
    public void AddEssenceOutput(int essenceId, float probability) {
