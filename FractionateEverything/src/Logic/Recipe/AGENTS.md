...
└── RectificationRecipe  — 精馏 (矩阵/黑雾矩阵 → 残片，作为特殊成长型稳定副资源入口)
...
- `RectificationRecipe` overrides `GetOutputs()` to output Fragments directly; it is no longer the old ticket/essence/core byproduct path
