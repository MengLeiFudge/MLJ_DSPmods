- v3.0.0
    - 重构主玩法，移除点金塔、分解塔、量子复制塔，收束为分馏、转化、数据中心与配方成长主线。
    - 重做抽取成长系统，收束为主抽取、抽取偏好、成长规划、流派聚焦与抽取总览，抽取消耗改为当前阶段矩阵，奖励直接进入分馏数据中心。
    - 主抽取路线奖励改为按资源组、转化链、塔种、精馏家族或单配方作为抽取单位结算，抽中组、链或家族时优先推进未解锁、未满级的底层配方，减少逐条抽取基础资源和链条配方的负担。
    - 重做配方成长体系，分馏配方改为解锁、升级、残片补偿与成长角色分工，配方效果围绕等级、原料保留、产物翻倍和图鉴加成重新结算。
    - 抽取单位回响开始参与实际分馏效果：矿物复制类和塔种偏向提高产物翻倍，转化链偏向提高输入保留，精馏家族偏向降低萃取损耗或提高调相输出；配方页会显示所属抽取单位的回响等级。
    - 精馏配方等级开始影响矩阵萃取损耗、精华压缩倍率和精华回流倍率，使矩阵精华线从实体闭环进一步变成可成长产线。
    - 修复矿物复制塔零压循环优先级：同类复制产物会先回补塔内自循环输入，满足循环后才进入流动输出和正面产物输出。
    - 修复通过交互塔上传分馏塔后像是直接消失的问题：分馏塔上传后进入数据中心实际可提取库存，库存达到1000后由分馏献祭按秒消耗当前库存的一部分来提供当秒加成。
    - 新增原版配方强化入口，原版配方页可查看和操作可强化配方，并按阶段开放对应增强。
    - 新增动态市场与残片经济，市场总览、市场板、市场指数、稳定兑换分别承担供需观察、限时补给、指数展示和有限救急；新增记忆源点作为低频珍贵货币，和残片分离。
    - 重做任务与成就系统，主线任务改为路线图，成就改为自动获得并提供长期加成；阶段主线完成和高阶成就会给予记忆源点，移除循环任务，避免固定目标在后期被高速重复领取。
    - 增加速通模式与黑雾成长分支，成长规划、任务和重要物品展示会读取黑雾阶段信息，但黑雾系统本身只提供战斗事实。
    - 重组主面板页面分类，新增分析面板风格，统一生产管理、抽取成长、资源管理、任务成就、图鉴档案和系统设置等入口。
    - 重构数据中心与主要页面布局，统一卡片、网格、字号、间距和按钮状态，改善物品交互、配方图鉴、建筑操作、市场板等页面的可读性。
    - 优化分馏塔与物流交互站相关操作，补齐建筑培养、单路锁定、流动输出尾料释放、上传路径和增产点数处理等逻辑；精馏塔改为矩阵精华萃取、压缩、回流和拆票闭环，有限上传目标数量会随物品价值调整。
    - 清理旧奖券、VIP、符文、旧成长货币等废弃主路径残留，保留必要的存档兼容壳。
    - 修复多项迁移与边界问题，包括跨存档状态污染、读档补齐、抽卡保底时序、主线悬浮提示、下拉框销毁、沙盒生成越界和页面刷新状态。
