- v3.0.0
    - 将主玩法重构为远古文明解析：交互塔、解析塔、资源塔和转化塔分别承担原胚培养与上传、矩阵解析与谱系分化、原矿复制和工业链转换。
    - 建立六个矩阵阶段的实体解析数据与递增检索机会；单次检索可能失败、发现协议或推进完整度，并提供失败保底、新发现保底和优先协议选择。
    - 资源复制与转化配方改由文明协议管理，完整度达到 100 后才可运行；建筑培养和矩阵解析继续作为基础设施配方。
    - 建立唯一的远古文明科技点和四条纵向塔型科技主干，每条依次包含流动输出堆叠、产物输出堆叠、分馏永动、主路锁定和副产物弃置。
    - 接入四条科技主干的全部五项运行能力。主路锁定和副产物弃置采用“塔型科技许可、配方累计成功校准、单塔实例设置”三层约束，并同步存档、复制粘贴、蓝图和 Nebula 状态。
    - 主路锁定只选择主产物；副产物弃置只销毁副产物且不提供补偿。批量分馏只按实际产生过产物的成功次数推进配方校准。
    - 四塔的五项能力分别保持独立，但每一项都使用相同结构覆盖四塔；配方当前无收益时显示无效果状态，不再省略能力。四塔主路目标统一使用同一实例状态，旧解析谱系状态和蓝图参数在导入时兼容迁入。
    - “显示分馏配方详细信息”改名为“始终显示完整分馏配方信息”，并接入四塔共用的运行窗口概率显示。
    - 协议恢复提供广域、方向和锚定三种检索策略；方向和锚定统一消耗残片，锚定保证选中目标发生发现或推进；记忆源点不再作为 active 物品或货币，失败检索提供残片，批量入口按单次规则逐次结算。
    - Nebula 使用主机权威的文明动作与状态快照，同步协议、解析、科技、成就、配方校准和残片余额；客户端只提交动作请求，玩家加入时由主机补发当前状态。
    - 建立通用原胚随机孵化、四种专属原胚稳定培养、解析塔谱系分化和满足培养资格后的实体塔返祖协议。
    - 重新分配 FE 自有物品、配方、模型和隐藏科技 ID；只迁移 FE 2.x 中六类现役建筑及明确对应状态，不把量子复制、点金、旧等级成长等废弃内容映射为新机制。
    - 删除建筑等级、经验、突破、献祭、裂变池、共鸣和物流自动喷涂池；物流下载保留库存物品原有增产点，不再自动补喷。
    - 资源塔以同种资源自我复制为主产物，保留硅石、石矿、煤矿、氢等配方既定的自然伴生副产物，并排除临界光子和反物质。
    - 新增单存档文明成就，以协议恢复、阶段完成、科技投入和真实分馏次数作为固定里程碑并自动提供加成。
    - 重组主面板为生产管理、文明解析、资源管理、恢复指引、图鉴档案和系统设置；文明解析包含总览、协议恢复、远古科技树和文明成就四页。
    - 分馏配方页改为展示协议状态、配方产物和当前运行参数；物品或塔型切换后会自动选择实际存在的配方，不再出现可避免的“配方不存在”。
    - 修复配方目录初始化完成前的空查询被永久缓存，导致解析塔在配方已经可用后仍持续搬运且不产出的问题。
    - 没有必需协议的阶段会直接把检索机会投入深层解析，不再阻塞科技点获取。
    - 废弃首个文明框架试验使用的 `Civilization` 存档块；新系统只读取 `AncientCivilization`，旧协议状态不折算、不迁移并直接重新开始。
    - 删除旧抽取、商店、市场、兑换、旧任务与成就、旧统计、全局成长、建筑操作和线性配方成长代码，不保留隐藏页面或旧存档兼容块。
