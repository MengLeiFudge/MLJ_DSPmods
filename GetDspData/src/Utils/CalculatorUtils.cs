...
public static partial class Utils {
    ...
    public static int[] getAcceptFactories(this RecipeProto recipe) {
        if (GenesisBookEnable) {
            return (int)recipe.Type switch {
                ...
                (int)ERecipeType_GB.Particle => [I微型粒子对撞机, IGB苍穹粒子加速器],
                (int)ERecipeType_GB.PhotonStore => [I射线接收站],
                ...
                (int)ERecipeType_GB.星际组装厂 => [I巨构星际组装厂],
                _ => throw new($"配方类型异常，配方名称{recipe.name}，配方类型{recipe.Type}")
            };
        } else if (OrbitalRingEnable) {
            return (int)recipe.Type switch {
                (int)ERecipeType_OR.Smelt => [I电弧熔炉, IOR等离子熔炉, I负熵熔炉, IOR轨道熔炼站],
                (int)ERecipeType_OR.Chemical => [I化工厂, I量子化工厂],
                (int)ERecipeType_OR.Refine => [I原油精炼厂],
                (int)ERecipeType_OR.Assemble => [IOR基础制造台, IOR高速装配线, IOR粒子打印车间, IOR物质重组工厂],
                (int)ERecipeType_OR.Particle => [I微型粒子对撞机, IOR星环对撞机总控站],
                (int)ERecipeType_OR.PhotonStore => [I射线接收站],
                (int)ERecipeType_OR.Fractionate => [I分馏塔],
                (int)ERecipeType_OR.太空船坞 => [IOR太空船坞],
                (int)ERecipeType_OR.粒子打印 => [IOR粒子打印车间, IOR物质重组工厂],
                (int)ERecipeType_OR.等离子熔炼 => [IOR等离子熔炉, I负熵熔炉],
                (int)ERecipeType_OR.物质重组 => [IOR物质重组工厂],
            ...
            return (int)recipe.Type switch {
                (int)ERecipeType_DSP.Smelt => [I电弧熔炉, I位面熔炉, I负熵熔炉],
                (int)ERecipeType_DSP.Chemical => [I化工厂, I量子化工厂],
                (int)ERecipeType_DSP.Refine => [I原油精炼厂],
                (int)ERecipeType_DSP.Assemble => [I制造台MkI, I制造台MkII, I制造台MkIII, I重组式制造台],
                (int)ERecipeType_DSP.Particle => [I微型粒子对撞机],
                (int)ERecipeType_DSP.PhotonStore => [I射线接收站],
                (int)ERecipeType_DSP.Fractionate => [I分馏塔],
                (int)ERecipeType_DSP.Research => [I矩阵研究站, I自演化研究站],
                (int)ERecipeType_DSP.星际组装厂 => [I巨构星际组装厂],
                _ => throw new($"配方类型异常，配方名称{recipe.name}，配方类型{recipe.Type}")
            };
        }
    }
    ...
    public static float GetSpace(this ItemProto item) {
        return item.ID switch {
            ...
            I分馏塔 or IGB聚束液体汲取设施 or IFE交互塔 or IFE矿物复制塔 or IFE点数聚集塔 or IFE转化塔 or IFE精馏塔 => 12.96f,
            I微型粒子对撞机 => 45.12f,
            I能量枢纽 => 64f,
