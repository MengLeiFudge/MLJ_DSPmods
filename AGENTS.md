...
- For any `FractionateEverything` / `GetDspData` / `SaveDataExporter` / `UXAEnhance` / shared infrastructure change, after the verified code is committed, accepted, and merged back into the target branch, run the Debug solution build in the main Windows-mounted checkout, then run `AfterBuildEvent.exe 1` from `AfterBuildEvent\bin\Debug`. Expected behavior: copy built mod files to R2, create zip packages under `ModZips`, notify qqbot through the generic local `publish-local` admin API, and deliver every configured zip to its target QQ group without opening Explorer or launching Dyson Sphere Program on success. The final Codex reply must include the build command/result, AfterBuildEvent command/result, generated zip file paths, R2 copy status, qqbot delivery status, the uploaded commit hash, and the commit body used as the publish message.
...
- `FractionateEverything/DESIGN.md` is a human-facing implementation blueprint for the FE fractionation domain, not a short marketing or progress summary.
- Keep `DESIGN.md` focused on stable system hierarchy, design purpose, source-level class relationships, gameplay contracts, formulas, tables, recipe families, tower traits, draw/acquisition rules, task/achievement relationships, and major boundaries needed for a human owner to reconstruct the current fractionation design.
- Do not add build/publish commands, validation logs, commit history, temporary gaps, AI workflow instructions, task plans, or chat-derived status reports to `DESIGN.md`.
...
- When updating `DESIGN.md`, prefer stable implementation/design facts over brevity. Remove noisy process details, but keep the formulas and data tables that define current behavior.
